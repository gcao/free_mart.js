...
(function() {
  var CallbackRegistry, FreeMartInternal, FuzzyRegistry, HashRegistry, InUse, NOT_FOUND, NOT_FOUND_FINAL, NO_PROVIDER, Provider, Registry, SimpleRegistry, VERSION, extend, isDeferred, stringify, stringifyCache, stringifyCallback, toString,
    ...
    Registry.prototype.add = function(key, provider) {
      ...
      if (last instanceof HashRegistry && typeof key === 'string' && !last.accept(key)) {
        ...
        if (typeof key === 'string') {
          ...
          child_registry = new CallbackRegistry(this.market, key, provider);
        } else if (typeof key === 'undefined') {
          child_registry = new SimpleRegistry(this.market, provider);
        } else {
  ...
  CallbackRegistry = (function() {
    extend(CallbackRegistry.prototype, InUse);

    function CallbackRegistry(market, callback, provider) {
      this.market = market;
    ...
    CallbackRegistry.prototype.accept = function(key) {
      this.market.log("CallbackRegistry.accept", key);
      return this.callback(key);
    };

    CallbackRegistry.prototype.process_ = function() {
      var args, options, _ref, _ref1;
      options = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      (_ref = this.market).log.apply(_ref, ["CallbackRegistry.process_", options].concat(__slice.call(args)));
      if (!this.accept(options.$key)) {
        return NO_PROVIDER;
      }
      try {
  ...
  SimpleRegistry = (function() {
    extend(SimpleRegistry.prototype, InUse);

    function SimpleRegistry(market, provider) {
      this.market = market;
      this.provider = provider;
      this.in_use_keys = [];
    }

    SimpleRegistry.prototype.process_ = function() {
      var args, options, _ref, _ref1;
      options = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      (_ref = this.market).log.apply(_ref, ["SimpleRegistry.process_", options].concat(__slice.call(args)));
      try {
        options.$provider = this.provider;
        return (_ref1 = this.provider).process.apply(_ref1, [options].concat(__slice.call(args)));
      } finally {
  ...
  Provider = (function() {
    ...
    Provider.prototype.deregister = function() {
      return this.market.registry.removeProvider(this);
    };
  ...
  FreeMartInternal = (function() {
    ...
    FreeMartInternal.prototype.register = function(key, options, value) {
      var provider, request, result, _i, _len, _ref, _ref1;
          ...
          if (result === NOT_FOUND) {
            ...
            (function(request) {
              ...
              successCallback = function() {
                ...
                return request.resolve.apply(request, v);
              };
              failureCallback = function() {
                var v;
                v = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
                return request.reject.apply(request, v);
              };
              return result.then(successCallback, failureCallback);
            })(request);
          } else {
    ...
    FreeMartInternal.prototype.factory = function(key, value) {
      this.log('factory', key, value);
      return this.register(key, {
        $type: 'factory'
      }, value);
    };
